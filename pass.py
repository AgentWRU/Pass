...
def first_run_config(master_password):
    ...
    print
    "{*} Logging into Github now..."
    gh, repo, branch = git_connect(github_username=github_username, github_password=github_password,
                                   github_repo=github_repo)
    print
    "{*} Creating blank password store..."
    commit = repo.create_file("gitpass", "initiate",
                              base64.b64encode('{"creds":[]}'))
...
Reads in the config file stored in ~/.gitpass.conf, decrypts the github password, and returns the our_salt,gh,repo,branch objects for use.
    ...
    github_password = decrypt(master_password=master_password, our_salt=our_salt,
                              data=configuration['github_password'])  # death to plaintext
    github_repo = configuration['github_repo']  # we might encrypt this too, for shits and giggles
    gh, repo, branch = git_connect(github_username=github_username, github_password=github_password,
                                   github_repo=github_repo)  # this needs exception handling
    return our_salt, gh, repo, branch
...
Pushes our data to the github repo. Stores it in a file named "gitpass", obviously. Returns True if success, or False if failure.
    """
    try:
        commit = repo.contents(path="gitpass").update(message="lol",
                                                      content=base64.b64encode(str(data)))  # fucking bodge
    except Exception as e:
        print(e)
        "{!!} Oh no! Something wen't wrong! Try again later!"
...
Pulls our data from the github repo. Only looks for files named "gitpass" in "master". Returns data if success, or fails and some other fucking shit handles the error.
    """
    data = base64.b64decode(base64.b64decode(repo.contents(path="gitpass").content))
    return data.strip()
...
Decrypts data with our salt and master password for extreme levels of security!!!
    ...
    key = base64.urlsafe_b64encode(kdf.derive(master_password))
    f = Fernet(key)
    data_enc = binascii.unhexlify(data.encode('hex'))
...
# this is the 'raw', unencrypted, keystore. Passwords are decrypted on access:
{
    "creds":[
             ...
             "password": "youup",
             "datetime": "2023-10-23 21:58:22.375668"
             },
             {
...
Inserts a set of credentials into the password store.
    ...
    new_record = {"site": encrypted_site, "username": encrypted_username, "password": encrypted_password,
...
Updates a password record in the data store (by adding a new record to it and sorting it, so we have history!)
    ...
    password_store['creds'].append(credentials)
    print
    "{*} Updated!"
    print
    "{!} You may want to run 'list' again buddy..."
    return password_store
...
Later, we can add such nice things as "check the date and warn on 'expired' passwords!"
    ...
    for record in records:
        ...
        print
        "[%s] user: %s site: %s" % (index, username_decrypted, webshite_decrypted)
    return True
...
This is the interactive "shell".
    """
    print
    "Welcome to the GitPass Console. Type 'help' for help."
    password_store = ast.literal_eval(git_pull(gh, repo, branch))
    while True:
        try:
        ...
        if command == "insert":
            ...
            password_store = insert_password(master_password=master_password, our_salt=our_salt, username=username,
                                             password=password, webshite=webshite, password_store=password_store)
        if command == "retrieve":
            index = raw_input("Index of password you want to retrieve: ")
            username, password, webshite = retrieve_password(master_password=master_password, our_salt=our_salt,
                                                             index=index, password_store=password_store)
            print
            "{>} Website: %s" % (webshite)
            print
            "{>} Username: %s" % (username)
        ...
        if command == "update":
            ...
            password_store = update_password(master_password=master_password, our_salt=our_salt, index=index,
                                             password_store=password_store, password=password)
        if command == "delete":
            index = raw_input("Index number of password you wish to delete: ").strip()
            password_store = delete_password(master_password=master_password, our_salt=our_salt, index=index,
